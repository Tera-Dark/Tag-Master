...
import { Project, TagImage, AppSettings, PromptTemplate, DEFAULT_TEMPLATES, DEFAULT_PROMPT, WorkflowStep } from './types';
import { Upload, FolderInput, X } from './components/Icons';
...
const App: React.FC = () => {
    ...
    const handleTestConnection = async (testSettings: AppSettings) => {
        try {
            ...
            addToast(`${t('connectionFailed')}: ${error.message}`, 'error');
            throw error;
        }
    };
    ...
    }, [handleSelectAll, clearSelection, multiSelection, selectedId, removeImages, t, currentStep]);
                ...
                */}
                ...
                <div className="flex-1 flex flex-col min-w-0 overflow-hidden relative">
                    {currentStep === WorkflowStep.PREPROCESS ? (
                        ...
                        <ReviewView projects={projects} t={t} onNext={handleNextStep} />
                    ) : currentStep === WorkflowStep.EXPORT ? (
                        <ExportView onExport={handleExport} totalImages={projects.reduce((acc, p) => acc + p.images.length, 0)} t={t} />
                    ) : (
                ...
                <BatchEditModal
                    ...
                    onBatchUpdate={(op: any, params: any, scope: 'all' | 'selected') => {
                        const targetIds = new Set(scope === 'selected' ? Array.from(multiSelection) : visibleImages.map(v => v.img.id));
                        if (targetIds.size > 0) {
