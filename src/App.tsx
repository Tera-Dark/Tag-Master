import { keyboardContext } from './utils/interaction';
import { useDialogs } from './components/ui/DialogContext';
import { EmptyWorkspace } from './components/EmptyWorkspace';
import React, { useState, useEffect, useMemo, useCallback } from 'react';
import { exportAllProjectsToZip, downloadSingleText } from './services/exportService';
import { WifiOff } from 'lucide-react';
import { WorkflowStep, Project } from './types';
...
const App: React.FC = () => {
  ...
  const {
    ...
    openFileDialog,
  } = useFileHandler(activeProjectId, addFilesToProject, addToast);

  // Check Tutorial
  useEffect(() => {
    if (!localStorage.getItem(TUTORIAL_SEEN_KEY)) setIsTutorialOpen(true);
  }, []);

  // Check API Configuration on mount
  useEffect(() => {
    // API setup is available from Settings and when starting a task.
    // Keep the first workspace visit focused on importing data.
  }, [settings.apiKey]);
  ...
  const contextProjects = useMemo(
    () => (activeProjectId === 'all' ? projects : projects.filter((p) => p.id === activeProjectId)),
  ...
  const virtualItems = useMemo(
    () => visibleImages.map((v) => ({ image: v.img, projectId: v.projId })),
    [visibleImages]
  );
  ...
  useEffect(() => {
    const handleKeyDown = async (e: KeyboardEvent) => {
      const context = keyboardContext(e, !!document.querySelector('[aria-modal="true"]'));
      ...
      if (
        currentStep === WorkflowStep.TAGGING ||
        ...
        if (e.key === 'Delete' || e.key === 'Backspace') {
          if (multiSelection.size > 0) {
            if (
              await confirm(
                t('deleteSelectedConfirm').replace('{count}', multiSelection.size.toString())
              )
            ) {
            ...
            if (await confirm(t('deleteSelectedConfirm').replace('{count}', '1'))) {
  ...
  const handleMoveConfirm = (targetProjectId: string) => {
    if (moveState.mode === 'project' && moveState.sourceProjectId) {
      ...
      const imageIds =
        multiSelection.size > 0 ? Array.from(multiSelection) : selectedId ? [selectedId] : [];
  ...
  const handleLightboxNav = (direction: 'next' | 'prev') => {
    ...
    let newIndex = -1;
    if (direction === 'next' && currentIndex < visibleImages.length - 1)
  ...
  const handleClean = (
    ...
    const targetIds = new Set(
      scope === 'selected' ? Array.from(multiSelection) : visibleImages.map((v) => v.img.id)
    );
    if (targetIds.size > 0) {
      if (op === 'applyRules') {
  ...
  const inspectorProjectObj = inspectorProjectId
    ? projects.find((p) => p.id === inspectorProjectId)
    : null;
  ...
  if (!isLoaded) {
    return (
      <main className='min-h-screen grid place-content-center gap-4 bg-zinc-950 text-zinc-100 p-8 text-center'>
        <p role={loadError ? 'alert' : 'status'}>
          {t(loadError ? 'storageLoadError' : 'storageLoading')}
        </p>
        {loadError && (
          <button
            className='tm-button rounded-lg bg-zinc-600 px-4 py-2'
  ...
  return (
    <div
      className={`tm-app ${isSidebarOpen ? 'tm-sidebar-is-open' : ''} h-screen w-screen flex flex-col bg-tm-canvas text-tm-text transition-colors duration-200 ${settings.theme === 'dark' ? 'dark' : ''}`}
    ...
    >
      ...
      {(saveStatus === 'unsaved' || (saveStatus === 'saving' && projects.length > 0)) && (
        <div role='status' className='tm-save-status' data-error={saveStatus === 'unsaved'}>
          <span>{t(saveStatus === 'saving' ? 'storageSaving' : 'storageSaveError')}</span>
          {saveStatus === 'unsaved' && (
            <button onClick={retrySave} className='tm-button underline'>
              {t('storageRetry')}
            </button>
          )}
        </div>
      ...
      {isOffline && (
        <div className='bg-amber-500/10 text-amber-600 dark:text-amber-500 text-xs font-medium py-1.5 px-4 flex items-center justify-center gap-2 border-b border-amber-500/20'>
          ...
          <span>
            {t('offlineWarning') ||
              'You are currently offline. Local features are still available, but AI generation is disabled.'}
          </span>
        </div>
      )}
      ...
      {isCooling && (
        <div className='    border-b border-amber-500/20 text-amber-700 dark:text-amber-300 px-4 py-2 flex items-center justify-between text-xs backdrop-blur-md animate-in fade-in duration-300'>
          <div className='flex items-center gap-2 font-medium'>
            <span className='flex h-2 w-2 relative'>
              <span className='animate-ping absolute inline-flex h-full w-full rounded-full bg-amber-400 opacity-75'></span>
              <span className='relative inline-flex rounded-full h-2 w-2 bg-amber-500'></span>
            </span>
            <span> 触发 API 速率限制 (5 RPM 或 Token 配额耗尽)，正在自动排队轮询恢复中...</span>
            <span className='font-mono font-semibold bg-amber-500/20 px-2 py-0.5 rounded-full text-amber-800 dark:text-amber-200'>
          ...
          <div className='flex items-center gap-2'>
            <button
              ...
              className='tm-button px-2.5 py-1 rounded-lg bg-amber-500/20 hover:bg-amber-500/30 text-amber-800 dark:text-amber-200 text-xs font-semibold transition-all cursor-pointer'
            >
              跳过等待立即重试
            </button>
            <button
              onClick={pause}
              className='tm-button px-2.5 py-1 rounded-lg bg-black/[0.05] dark:bg-white/[0.08] hover:bg-black/[0.1] text-tm-secondary text-xs font-semibold transition-all cursor-pointer'
            >
              暂停批处理
            </button>
          </div>
      ...
      <div className='flex-1 flex overflow-hidden'>
        ...
        {isSidebarOpen && (
          <button
            className='tm-mobile-scrim md:hidden'
        ...
        <Sidebar
          ...
          handlers={{
            ...
            onMerge: (sourceId) =>
              setMoveState({ isOpen: true, mode: 'project', sourceProjectId: sourceId }),
            onDeleteProject: (id) => {
          ...
          >
            {currentStep === WorkflowStep.PREPROCESS ? (
              ...
              <ExportView
                ...
                totalImages={projects.reduce((acc, p) => acc + p.images.length, 0)}
              />
            ) : (
              ...
              <>
                <SmartToolbar
                  ...
                  setViewMode={(m) => setSettings((s) => ({ ...s, viewMode: m }))}
                  ...
                  handlers={{
                    ...
                    onDeleteSelected: async () => {
                      if (
                        await confirm(
                          t('deleteSelectedConfirm').replace(
                            '{count}',
                            multiSelection.size.toString()
                          )
                        )
                      ) {
                        removeImages(new Set(Array.from(multiSelection)));
                    ...
                    onClearDone: async () => {
                      if (await confirm(t('clearDoneConfirm'))) clearDone();
                    },
                  }}
                  settings={settings}
                ...
                <div className='flex-1 flex min-w-0 relative overflow-hidden bg-tm-canvas'>
                  <div
                  ...
                  >
                    {visibleImages.length === 0 ? (
                      <EmptyWorkspace
                        filtered={
                          ...
                          (!!searchQuery.trim() || viewFilter !== 'all')
                        }
                        onImport={openFileDialog}
                    ...
                    {/* Drag Overlay */}
                    {isDragOver && (
                      <div className='absolute inset-0 bg-black/[0.03] dark:bg-white/[0.04] backdrop-blur-xs z-50 flex items-center justify-center border-2 border-dashed border-zinc-400 dark:border-zinc-600 m-3 rounded-xl animate-in fade-in'>
                        <div className='bg-tm-canvas p-6 rounded-xl shadow-none flex flex-col items-center border border-tm-border'>
                          <Upload className='w-8 h-8 text-tm-secondary mb-2' />
                  ...
                  {currentStep !== WorkflowStep.IMPORT && (
                    <Inspector
                      ...
                      onUpdateCaption={(text) =>
                        inspectorProjectId &&
                        activeImage &&
                        updateImageCaption(inspectorProjectId, activeImage.id, text)
                      }
                      onRename={(newName) =>
                        inspectorProjectId &&
                        activeImage &&
                        renameImage(inspectorProjectId, activeImage.id, newName)
                      }
                      onRegen={() =>
                        inspectorProjectId &&
                        activeImage &&
                        processSingle(inspectorProjectId, activeImage.id)
                      }
        ...
        <BatchEditModal
          ...
          selectedCount={multiSelection.size}
          onBatchUpdate={(
            op: 'append' | 'prepend' | 'replace' | 'removeTags' | 'addTags' | 'applyRules',
            params: Parameters<typeof batchUpdateCaptions>[2],
            scope: 'all' | 'selected'
          ) => {
            const targetIds = new Set(
              scope === 'selected' ? Array.from(multiSelection) : visibleImages.map((v) => v.img.id)
        ...
        <MoveModal
          moveState={moveState}
          onClose={() => setMoveState({ isOpen: false, mode: 'selection' })}
          projects={projects}
          selectionCount={multiSelection.size}
