...
export const VirtualList = ({
  ...
  onCardPointerDown: (id: string, e: React.PointerEvent | React.KeyboardEvent) => void;
  ...
  return (
    <div
      ...
      className='flex-1 overflow-y-auto p-4 custom-scrollbar relative outline-none animate-in fade-in duration-300 ease-out'
      tabIndex={-1}
    >
...
export const VirtualGrid = ({
  ...
  const gap = 20;
  // Narrow layouts adapt without changing the user's saved column preference.
  const contentWidth = Math.max(1, clientWidth - 32); // p-4 left + right
  const visibleColumns =
    clientWidth < 640
      ? Math.min(columnCount, Math.max(1, Math.floor((contentWidth + gap) / 160)))
      : columnCount;
  const itemWidth = (contentWidth - gap * (visibleColumns - 1)) / visibleColumns;
  const rowHeight = itemWidth + gap;

  const totalRows = Math.ceil(items.length / visibleColumns);
  ...
  const visibleItems = useMemo(() => {
    const startIndex = startRow * visibleColumns;
    const endIndex = endRow * visibleColumns;
    return items.slice(startIndex, endIndex).map((item, index) => ({
      ...item,
      absoluteIndex: startIndex + index,
    }));
  }, [items, startRow, endRow, visibleColumns]);
